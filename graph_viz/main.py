...
def collapse_graph(g):
    ...
    for node_name, node in g.nodes.items():
        ...
        # now this works
        while True:
            if cur_node.node_type in ("func", "args") and len(cur_users) == 1:
                ...
                first_node_or_node_args_one = (
                    ...
                    cur_node is node or _get_num_node_args(cur_node) == 1
                )
        ...
        if len(collapsible_subgraph_node_names) >= 3:
            ...
            collapse_node_start_to_node_names[node_name] = (
                collapsible_subgraph_node_names
            )

    for (
        node_start_name,
        collapsible_chain_names,
    ) in collapse_node_start_to_node_names.items():
        # example chain:
        ...
        new_node_snippets = [
            (node_start_name, g.nodes[node_start_name].args, collapsible_chain_names[0])
        ]
        for idx in range(1, len(collapsible_chain_names) - 1, 2):
            new_node_snippets.append(
                (
                    collapsible_chain_names[idx],
                    g.nodes[collapsible_chain_names[idx]].args,
                    collapsible_chain_names[idx + 1],
                )
            )
...
def parse_graph(
    ...
    for line in list_of_lines[start_idx : end_idx + 1]:
        ...
        if res:
            var_name, var_hint, func_name, args_str = (
                res.group(1),
                res.group(2),
                res.group(3),
                res.group(4),
...
def parse_triton_region_graph(
    ...
    for line in list_of_lines[start_idx : end_idx + 1]:
        ...
        if res:
            ...
            kernel_name_to_arg_idx_to_input_output_none[kernel_name] = (
                arg_idx_to_category
            )
        ...
        incoming_var_size_hint_re = re.compile(
            "[ ]+assert_size_stride\(([\w]+), (.*)\)"
        )
        ...
        buffer_reuse_re = re.compile(
            "[ ]+([\w]+) = reinterpret_tensor\(([\w]+), (.*)\);.*"
        )
        ...
        if res:
            ...
            arg_idx_to_input_output_none = kernel_name_to_arg_idx_to_input_output_none[
                kernel_name
            ]
        ...
        if res:
            ...
            g.nodes[cur_kernel_node_name] = Node(
                cur_kernel_node_name, input_args, {}, node_type="func"
            )
            metadata = {"hint": node_name_to_hint[output_arg]}
            g.nodes[output_arg] = Node(
                output_arg, [cur_kernel_node_name], metadata=metadata, node_type="args"
            )
...
def create_diagram(
    ...
    for node_name, node in g.nodes.items():
            ...
            for k, v in node.metadata.items():
                if k == "snippets":
                    ...
                    for snippet in v:
                        ...
                        metadata_str += (
                            f"{new_node_name} = {prev_node_name}({prev_args}),<br/>"
                        )
                    metadata_str += "],"
                else:
        ...
        if node.node_type == "args":
            ...
            node_comment = (
                f"<{shorten_func_name(node_name)}({args_str})<br/>{metadata_str}>"
            )
...
def run(
    ...
    * `fname`: filename with logs
    ...
    joint_start_idx, forward_start_idx, backward_start_idx, end_idx = (
        None,
        None,
        None,
        None,
    ...
    with open(fname, "r") as f:
        ...
        for idx, line in enumerate(lines):
            ...
            # ===== Forward graph 0 =====
            start_of_aot_forward_graph_re = re.compile(
                " ===== Forward graph ([\d]+) ====="
            )
            ...
            # ===== Forward graph 0 =====
            start_of_aot_backward_graph_re = re.compile(
                " ===== Backward graph ([\d]+) ====="
            )
            ...
            if end_of_aot_graph_re.match(line):
                end_idx = idx
                aot_graph_id_to_graphs[cur_captured_graph_id]["joint"] = (
                    joint_start_idx,
                    end_idx,
                )
                cur_captured_graph_id = None
            ...
            if end_of_aot_forward_graph_re.match(line):
                ...
                if forward_start_idx is not None:
                    aot_graph_id_to_graphs[cur_captured_graph_id]["forward"] = (
                        forward_start_idx,
                        end_idx,
                    )
                elif backward_start_idx is not None:
                    aot_graph_id_to_graphs[cur_captured_graph_id]["backward"] = (
                        backward_start_idx,
                        end_idx,
                    )
                else:
            ...
            if (
                end_of_triton_kernel_file.match(line)
                and aot_triton_region_start_idx is not None
            ):
                aot_triton_region_end_idx = idx
                graph_id_to_fwdbwd_to_triton_region_idxs[cur_aot_id][cur_fwd_bwd] = [
                    aot_triton_region_start_idx,
                    aot_triton_region_end_idx,
                    None,
                ]
                aot_triton_region_start_idx, aot_triton_region_end_idx = None, None
    ...
    for aten_graph_id, graphs in aot_graph_id_to_graphs.items():
        ...
        if "forward" in graph_id_to_fwdbwd_to_triton_region_idxs[aten_graph_id]:
            start_idx, end_idx, _ = graph_id_to_fwdbwd_to_triton_region_idxs[
                aten_graph_id
            ]["forward"]
            triton_region_graph_forward = parse_triton_region_graph(
                lines, start_idx, end_idx
            )
            aten_graph_id_to_name_to_graph[aten_graph_id]["triton_forward"] = (
        ...
        if "backward" in graph_id_to_fwdbwd_to_triton_region_idxs[aten_graph_id]:
            ...
            triton_region_graph_backward = parse_triton_region_graph(
                lines, start_idx, end_idx
            )
            aten_graph_id_to_name_to_graph[aten_graph_id]["triton_backward"] = (
