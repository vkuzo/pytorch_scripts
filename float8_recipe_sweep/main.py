...
def run():
    ...
    for K, w_outlier_val in itertools.product(Ks, outlier_vals):
        print(
            f"M {M}, K {K}, N {N}, x_outlier_val {x_outlier_val}, w_outlier_val {w_outlier_val}, go_outlier_val {go_outlier_val}"
        )
        ...
        for recipe_name in ("tensorwise", "rowwise", "rowwise_with_gw_hp"):
            ...
            gw_mape = mean_absolute_percentage_error(
                m_ref_copy.weight.grad, m.weight.grad
            ).item()
