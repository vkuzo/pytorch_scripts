...
def run(in_file: str):
    ...
    assert len(results) < 1000, (
        "github CLI has a limit of 1000 returned results, and we are at this limit"
    )
    ...
    repo_to_count = sorted(
        repo_to_count.items(), key=lambda item: item[1], reverse=True
    )
    ...
    print(
        "cumulative non-pytorch number of callsites", sum(x[1] for x in repo_to_count)
    )
